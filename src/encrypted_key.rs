...
impl EncryptedKey {
    ...
    /// Returns the indicator (always 0x60)
    pub fn indicator() -> u8 {
        0x60
    }
    ...
    /// Returns the hashed password prefix (always "16")
    ...
    fn hash_secret(salt: &[u8], secret: &str) -> [u8; 20] {
        ...
        println!("{:?} {:?} {:?}", salt, secret.as_bytes(), input);
        while count > 0 {
            if count > secret_len {
                sha.update(&input);
    ...
    pub fn try_convert<S: AsRef<str>>(s: S) -> Result<Self, ()> {
        if s.as_ref().len() != 61 {
            return Err(());
        }
        ...
        hex::decode_to_slice(&s.as_ref().as_bytes()[3..], &mut bytes).map_err(|_| ())?;
...
mod test {
    ...
    fn with_salt() {
        ...
        let key = EncryptedKey::hash_with_salt("come_gently_into_the_night_with_a_bag_of_rabbits?!", hex!("72FAE4A168BFD4B9"));
        assert_eq!(key.to_string().as_str(), "16:72FAE4A168BFD4B960381AF76E8204863A3A3C2FE96D48B4D6D92B9D16");
    }
